...
mkdir -p "$logDir"
logFile="$logDir/club-$(date +%Y%m%d-%H%M%S).log"

{
  echo "Club capture started $(date -Is)"
  ...
  /usr/bin/flock -n -o "$logDir/run.lock" dotnet "$dll" --capture-club --no-shutdown
  echo "Club capture finished $(date -Is)"
} >> "$logFile" 2>&1
